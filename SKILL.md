...
description: "通用方法论：从网页驾驶舱/大屏/工作台项目产出完整数据底座（不绑定任何行业业务）。① 依次清点每个页面的每个图表（标题、口径卡数据来源、JS数据结构、图表类型）② 为每个图表设计所需数据表并输出每页一个 md ③ 统筹全站数据表（按域分组+ER关联+标准系统表）④ 设计不重复、逻辑正确的数据库（事实表+派生视图+受控冗余+统一字典+快照主-明细）⑤ 生成带全量中文表/列注释的 PostgreSQL DDL 并导入本地 PG（连接参数化）⑥ 分域导出好看的 ER 图（Mermaid PNG + dbdiagram.io DBML + HTML 画廊）。适用于任何业务领域：驾驶舱数据建模、图表数据需求统计、数据库设计、DDL建库导入、ER图导出、一人一档类系统设计。业务实体（如教师/客户/学员/设备）由使用者映射，本 skill 只提供方法。"
...
> 参考实现：`references/EXAMPLE.md`（教育领域"人才罗盘"项目落地案例，含 table/ 30 页文档 + 64 表 DDL + 9 张分域 ER 图，业务名词仅作示范）。
...
模板：`templates/统筹总表模板.md`。含：页面→数据表索引、按业务域分组的全站表清单、ER 核心链路（ASCII）、标准系统表、口径统一说明。**统一命名**：同一概念全站一张表，禁止"一图一表"。

## 阶段 4 · 数据库设计 → `数据库设计.md`

模板：`templates/数据库设计模板.md`。四条"不重复"法则（与业务无关，任何领域通用）：
